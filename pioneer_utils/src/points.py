...
#################################################################AQUI TOCA LEER DE ARCHIVO Y METER A UN DICCIONARIO
 locations = dict()
...
def make_plan_client(start_x, start_y, goal_x, goal_y):
    rospy.wait_for_service('make_plan')
    try:
        make_plan = rospy.ServiceProxy('make_plan', planService)

        startPose = PoseStamped()
        startPose.header.stamp = rospy.Time.now()
        startPose.header.frame_id = 'map'
        startPose.pose = (Pose(Point(start_x, start_y, 0.000), Quaternion(0.000, 0.000, 0.000, 0.000))

        goalPose = PoseStamped()
        goalPose.header.stamp = rospy.Time.now()
        goalPose.header.frame_id = 'map'
        goalPose.pose = (Pose(Point(goal_x, goal_y, 0.000), Quaternion(0.000, 0.000, 0.000, 0.000))

        path = make_plan(startPose, goalPose)
